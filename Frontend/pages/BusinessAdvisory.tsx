        ...
        return (
            <div className="p-8 max-w-7xl mx-auto flex items-center justify-center min-h-[80vh]">
                <div className="text-center py-20 px-10 bg-white rounded-[48px] border border-[#E6E6E6] shadow-xl max-w-3xl w-full">
                    ...
                    <button
                        onClick={() => setStep(1)}
                        className="px-8 py-4 bg-[#22C55E] text-white rounded-2xl font-bold text-xl hover:bg-[#16A34A] transition-all shadow-lg hover:scale-105 flex items-center justify-center gap-3 mx-auto"
                    >
    ...
    if (step === 1) {
        return (
            <div className="p-4 md:p-8 max-w-4xl mx-auto">
                <button onClick={() => setStep(0)} className="flex items-center gap-2 text-[#555555] font-bold mb-6 hover:text-[#1F5F4A] transition-colors">
                ...
                <div className="bg-white rounded-[32px] border border-[#E6E6E6] shadow-xl p-6 md:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-4 bg-[#FAFAF7] border border-[#E6E6E6] rounded-2xl focus:outline-none focus:border-[#1F5F4A] placeholder-gray-400 font-medium transition-all"
                        ...
                        */}
                        ...
                        <div>
                            <div className="flex justify-between items-end mb-2 ml-2">
                                ...
                                {formData.totalLand && parseFloat(formData.totalLand) > 0 && (
                                    <span className="text-xs font-bold text-[#1F5F4A] bg-[#E6F4EA] px-2 py-1 rounded-lg">
                            ...
                            <input
                                ...
                                className={`w-full p-4 bg-[#FAFAF7] border rounded-2xl focus:outline-none placeholder-gray-400 font-medium transition-all ${(parseFloat(formData.businessLand) > parseFloat(formData.totalLand))
                                    ...
                                    : 'border-[#E6E6E6] focus:border-[#1F5F4A]'
                                    }`}
                        ...
                        <div>
                            ...
                            <select
                                className={`w-full p-4 bg-[#FAFAF7] border border-[#E6E6E6] rounded-2xl focus:outline-none focus:border-[#1F5F4A] font-medium transition-all ${!formData.waterSource ? 'text-gray-400' : 'text-[#1E1E1E]'}`}
                            ...
                            {interestOptions.map(interest => (
                                <button
                                    ...
                                    className={`p-4 rounded-2xl border text-sm font-bold transition-all ${formData.interests.includes(interest)
                                        ? 'bg-white border-[#1F5F4A] shadow-md ring-1 ring-[#1F5F4A] text-[#1F5F4A]'
                                        : 'bg-white border-[#E6E6E6] text-[#555555] hover:bg-stone-50'
                                        }`}
                                >
                                    <div className={`w-4 h-4 rounded-full border mb-2 mx-auto transition-colors ${formData.interests.includes(interest) ? 'bg-[#1F5F4A] border-[#1F5F4A]' : 'border-gray-300'
                                        }`}
                                    />
                                    {interest}
                                </button>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {['Village only', 'Small town within 10 km', 'City within 30 km', 'Direct buyers already available'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.marketAccess === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6] hover:border-gray-400'}`}>
                                            <input
                                                type="radio"
                                                name="marketAccess"
                                                value={opt}
                                                ...
                                                className="w-4 h-4 text-[#1F5F4A] focus:ring-[#1F5F4A] accent-[#1F5F4A]"
                                            />
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    {['Yes, I can handle customers', 'Maybe, with guidance', 'No, I prefer bulk buyers only'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.sellingPreference === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6]'}`}>
                                            <input
                                                type="radio"
                                                ...
                                                className="w-4 h-4 text-[#1F5F4A] accent-[#1F5F4A]"
                                            />
                                            <span className="ml-3 text-sm font-medium text-gray-700">{opt}</span>
                                        </label>
                                    ))}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    {['Very comfortable', 'Somewhat okay', 'Not comfortable at all'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.incomeComfort === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6]'}`}>
                                            <input
                                                type="radio"
                                                name="incomeComfort"
                                                value={opt}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                    {['Less than 1 year', '1-2 years', '2-3 years', 'I can wait longer'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.investmentRecovery === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6]'}`}>
                                            <input
                                                type="radio"
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {['I understand initial losses are possible', 'Small losses acceptable', 'I cannot afford losses'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.lossAttitude === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6]'}`}>
                                            <input
                                                type="radio"
                                                name="lossAttitude"
                                                value={opt}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {['Safe income, lower profit', 'Higher profit, higher risk'].map(opt => (
                                        <label key={opt} className={`flex items-center p-3 bg-white rounded-xl border cursor-pointer transition-all ${formData.riskPreference === opt ? 'border-[#1F5F4A] ring-1 ring-[#1F5F4A]' : 'border-[#E6E6E6]'}`}>
                                            <input
                                                type="radio"
                                                name="riskPreference"
                                                value={opt}
                    ...
                    <div className="bg-[#FAFAF7] rounded-2xl p-6 border border-[#E6E6E6]">
                        ...
                        <button
                            ...
                            className={`w-full py-4 bg-[#86C898] text-white rounded-xl font-bold text-xl transition-all shadow-md hover:scale-[1.01] flex items-center justify-center gap-2 ${loading ? 'opacity-75 cursor-not-allowed' : 'hover:bg-[#72b485]'}`}
                        >
        ...
        return (
            <div className="p-4 md:p-8 max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <button onClick={() => setStep(1)} className="flex items-center gap-2 text-[#555555] font-bold hover:text-[#1F5F4A] transition-colors">
                        <ArrowLeft className="w-5 h-5" /> Back
                    </button>
                    <button onClick={() => setStep(1)} className="text-[#22C55E] font-bold hover:underline">
                        Retake Assessment
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {recommendations.map((rec) => (
                        <div key={rec.id} className="bg-white rounded-[32px] border border-[#E6E6E6] overflow-hidden flex flex-col hover:shadow-2xl transition-all duration-300 group">
                            {/* Card Header */}
                            <div className="bg-[#22C55E] p-6 relative overflow-hidden">
                            ...
                            <div className="p-6 flex-grow flex flex-col">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold text-[#22C55E] uppercase tracking-widest mb-1">Profit</p>
                                        <p className="text-[#1F5F4A] font-bold text-sm">{rec.profit_potential}</p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-auto flex flex-col gap-3">
                                    <button
                                        ...
                                        className="w-full py-4 bg-[#1F5F4A] text-white rounded-xl font-bold text-lg hover:bg-[#184d3c] transition-all shadow-lg flex items-center justify-center gap-2 transform hover:scale-[1.02]"
                                    >
                                    ...
                                    <div className="flex gap-3">
                                        <button
                                            ...
                                            className="flex-1 py-3 bg-white border border-[#22C55E] text-[#22C55E] rounded-xl font-bold text-sm hover:bg-[#F0FDF4] transition-colors flex items-center justify-center gap-2"
                                        >
